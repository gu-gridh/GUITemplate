<template>
  <div style="text-align: center;">
    <div>
      <ButtonLink label="Apply filters" />
    </div>

    <ButtonLink icon="download" label="Download file" />
    <ButtonLink icon="down" label="Go to archive" />
    <ButtonLink icon="add" label="More" />
    <ButtonLink icon="info" label="Instructions" />
    <ButtonLink icon="eye" label="Hide annotations" />

    <div>
      <ButtonLink icon="left" label="Back" />
      <ButtonLink icon="right" label="About the project" />
    </div>

    <div>
      <ButtonLink icon="play" label="Play" />
      <ButtonLink icon="pause" label="Pause" />
    </div>

    <div>
      <ButtonLink icon="search" label="Search" />
      <ButtonLink icon="searchcircle" label="Search" />
    </div>

    <div>
      <ButtonLink icon="back" label="Previous page" />
      <ButtonLink icon="link" label="Next page" />
    </div>
  </div>
</template>
